...
import numpy as np
import abc
from inversion import ptp_utils, seq_aligner
...
print(
    f'Prompt-to-Prompt constants: LOW_RESOURCE={LOW_RESOURCE}, NUM_DDIM_STEPS={NUM_DDIM_STEPS}, MAX_NUM_WORDS={MAX_NUM_WORDS}'
)
...
class LocalBlend:
    ...
    def get_mask(self, maps, alpha, use_pool):
        ...
        if use_pool:
            maps = nnf.max_pool2d(maps, (k * 2 + 1, k * 2 + 1), (1, 1),
                                  padding=(k, k))
    ...
    def __init__(self,
        ...
        if substruct_words is not None:
            substruct_layers = torch.zeros(len(prompts), 1, 1, 1, 1,
                                           MAX_NUM_WORDS)
            for i, (prompt, words_) in enumerate(zip(prompts,
                                                     substruct_words)):
                if type(words_) is str:
                    words_ = [words_]
                for word in words_:
...
class AttentionControlEdit(AttentionStore, abc.ABC):
    ...
    def replace_self_attention(self, attn_base, att_replace, place_in_unet):
        if att_replace.shape[2] <= 32**2:
            attn_base = attn_base.unsqueeze(0).expand(att_replace.shape[0],
                                                      *attn_base.shape)
            return attn_base
        else:
            return att_replace
...
class AttentionRefine(AttentionControlEdit):
    ...
    def __init__(self,
        ...
        super(AttentionRefine,
              self).__init__(prompts, num_steps, cross_replace_steps,
                             self_replace_steps, tokenizer, device, local_blend)
...
def make_controller(prompts: List[str],
                    ...
                    device,
                    blend_words=None,
                    equilizer_params=None) -> AttentionControlEdit:
