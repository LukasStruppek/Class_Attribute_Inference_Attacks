...
from torchvision.models import densenet, inception, resnet, vision_transformer
from torchvision.transforms.transforms import (ColorJitter, RandomCrop,
...
class Classifier(BaseModel):
    ...
    def _build_model(self, architecture, pretrained):
        ...
        # Note: inception_v3 expects input tensors with a size of N x 3 x 299 x 299, aux_logits are used per default
            ...
            if self.num_classes != model.heads.head.out_features:
                # exchange the last layer to match the desired numbers of classes
                model.heads.head = nn.Linear(model.heads.head.in_features,
                                             self.num_classes)
            return model
    ...
    def fit(self,
        ...
        if enable_logging:
            ...
            wandb_config[
                'transformations'] = training_data.transform.transforms
            for t in training_data.transform.transforms:
                ...
                elif type(t) is RandomResizedCrop:
                    wandb_config['RandomResizedCrop'] = t.size
        ...
        def craft_adv_example(x_true,
            ...
            for step in range(steps):
                ...
                x = torch.min(torch.max(x, x_true - epsilon), x_true + epsilon)
                x = torch.clamp(x, *value_range)
            return x
        ...
        for epoch in range(num_epochs):
            ...
            for inputs, labels in tqdm(trainloader,
                ...
                # craft adversarial examples for adversarial training
                if robust_training:
                    inputs = craft_adv_example(inputs,
                                               labels,
                                               self,
                                               8 / 255,
                                               alpha=2 / 255,
